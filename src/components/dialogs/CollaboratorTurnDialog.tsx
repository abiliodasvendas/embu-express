...
export function CollaboratorTurnDialog({
  ...
  }, [open, turnToEdit, form, collaborator, empresas]);
  ...
  const toggleDia = (dia: number, checked: boolean) => {
    ...
    if (checked) {
      form.setValue("horarios", [
        ...current,
        { dia_semana: dia, hora_inicio: "08:00", hora_fim: "18:00", tolerancia_pausa_min: 60 }
      ]);
    } else {
      form.setValue("horarios", current.filter(h => h.dia_semana !== dia));
    }
  };
          ...
          <Form {...form}>
            <form id="collaborator-turn-form" onSubmit={form.handleSubmit(onSubmit)} className="lg:grid lg:grid-cols-12 lg:gap-10">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
                    {diasSemana.map((dia) => {
                      ...
                      const config = form.watch("horarios")?.find(h => h.dia_semana === dia.id);
                      const index = form.watch("horarios")?.findIndex(h => h.dia_semana === dia.id);
                      ...
                      return (
                        <div
                          key={dia.id}
                          className={cn(
                            "flex flex-col gap-3 p-4 border rounded-3xl transition-all duration-300 relative overflow-hidden",
                            isWorking 
                              ? "bg-white border-blue-200 shadow-[0_2px_12px_-4px_rgba(59,130,246,0.15)]" 
                              : "bg-gray-50/80 border-gray-100 opacity-[0.65] hover:opacity-100 grayscale-[0.2]"
                          )}
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              <Checkbox
                                ...
                                checked={isWorking}
                                disabled={isClosed}
                                onCheckedChange={(checked) => toggleDia(dia.id, !!checked)}
                                className={cn(
                                  "w-6 h-6 rounded-lg transition-all", 
                                  isWorking ? "data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600 shadow-md shadow-blue-500/20" : "border-gray-300"
                                )}
                              />
                              <div className="flex flex-col">
                                ...
                                {!isWorking && !isClosed && (
                                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">Não Trabalha</span>
                                )}
                                {isClosed && (
                            ...
                            {!isWorking && !isClosed && (
                              <div className="absolute right-6 opacity-30 pointer-events-none">
                                <Plane className="w-8 h-8 text-gray-400 rotate-45" />
                              </div>
                            )}
                          </div>

                          {/* Se trabalha, mostra os campos com animação simples CSS implícita */}
                          {isWorking && index !== undefined && index !== -1 && (
                            <div className="grid grid-cols-3 gap-3 pt-3 border-t border-gray-100 mt-1 animate-in fade-in zoom-in-95 duration-200">
                              <FormItem className="space-y-1.5 flex flex-col">
                                <FormLabel className="text-[10px] font-bold text-gray-500 uppercase tracking-wider pl-1">Início</FormLabel>
                                <FormControl>
                                  <Input
                                    placeholder="00:00"
                                    ...
                                    onChange={(e) => {
                                      const newHorarios = [...(form.getValues("horarios") || [])];
                              ...
                              <FormItem className="space-y-1.5 flex flex-col">
                                <FormLabel className="text-[10px] font-bold text-gray-500 uppercase tracking-wider pl-1">Fim</FormLabel>
                                <FormControl>
                                  <Input
                                    placeholder="00:00"
                                    className="h-10 text-sm font-mono text-center rounded-xl bg-gray-50/50 border-gray-200 focus:bg-white focus:ring-2 focus:ring-blue-500/20"
                              ...
                              <FormItem className="space-y-1.5 flex flex-col relative">
                                <FormLabel className="text-[10px] font-bold text-gray-500 uppercase tracking-wider pl-1">Pausa</FormLabel>
              ...
              <div className="lg:col-span-5 space-y-8 mt-10 lg:mt-0">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="relative z-10">
                          <FormLabel className="text-emerald-900 font-bold ml-1 text-sm">Data de Início (Cálculo Pro-rata) <span className="text-red-500">*</span></FormLabel>
                          <FormControl>
                       ...
                       <FormField
                         ...
                         render={({ field }) => (
                           <MoneyInput
                             ...
                             required={true}
                             labelClassName="text-emerald-900 font-bold ml-1 text-sm"
                             inputClassName="pl-12 h-12 rounded-2xl bg-white/70 backdrop-blur-sm border-emerald-200 focus:bg-white focus:ring-2 focus:ring-emerald-500/20 transition-all text-lg font-bold text-emerald-900"
                           />
                         )}
                       />
                    </div>
                ...
                <section>
                  <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <DollarSign className="w-4 h-4 text-red-500" /> Descontos e Acordos
                  </h3>
                  ...
                  <div className="bg-gradient-to-br from-red-50/50 to-red-100/30 p-5 sm:p-6 rounded-3xl border border-red-100/50 shadow-[0_2px_10px_-4px_rgba(239,68,68,0.05)] space-y-4 relative overflow-hidden">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <MoneyInput
                          field={field}
                          label="Adiantamento Prévio Feito"
                          labelClassName="text-red-900 font-bold ml-1 text-sm"
                          inputClassName="pl-12 h-12 rounded-2xl bg-white/70 backdrop-blur-sm border-red-200 focus:bg-white focus:ring-2 focus:ring-red-500/20 transition-all font-bold text-red-900"
                        />
                      )}
                    />
                    <p className="text-xs text-red-600/70 ml-1 font-medium leading-relaxed max-w-[90%]">
                      Este valor será descontado no próximo extrato financeiro do colaborador.
                    </p>
                  </div>
                </section>
