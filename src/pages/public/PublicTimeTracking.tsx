...
import { FilterX, Users, Timer, AlertTriangle, Search, CheckCircle2, Clock, PlayCircle, LogOut } from "lucide-react";
...
import { formatTime, formatMinutes } from "@/utils/ponto";
...
type ManagementStatus = 'LATE' | 'WORKING' | 'DONE' | 'WAITING' | 'ABSENT';
...
export default function PublicTimeTracking() {
    ...
    // Helper to determine management status
    const getManagementStatus = (record: any): ManagementStatus => {
        const isToday = format(date, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd');
        const isPast = !isToday && date < startOfToday();
    ...
    const processedRecords = useMemo(() => {
        return records?.map(r => ({
            ...r,
            mgtStatus: getManagementStatus(r)
        })) || [];
    }, [records, date]);
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredRecords.map((record) => {
                            ...
                            return (
                                <Card key={record.id} className="border-none shadow-sm rounded-2xl overflow-hidden group hover:shadow-md transition-all duration-300 relative">
                                    {/* Vertical Status Bar */}
                                    <div className={cn("absolute left-0 top-0 bottom-0 w-1.5", config.border.replace('border-', 'bg-'))} />
                                        ...
                                        <div className="flex flex-col h-full justify-between gap-4">
                                            ...
                                            <div className="flex items-end justify-between border-t border-gray-50 pt-3">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className={cn("text-base font-black leading-none", record.entrada_hora ? "text-gray-800" : "text-gray-200")}>
                                                ...
                                                <div className="space-y-1 text-right">
                                                    ...
                                                    <p className={cn("text-base font-black leading-none", record.saida_hora ? "text-gray-800" : "text-gray-200")}>
